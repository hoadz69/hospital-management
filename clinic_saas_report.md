...
Backend hiện tại chia service theo thư mục:
...
- Figma MCP đã cài và OAuth thành công cho Codex. Ngày 2026-05-08, sau khi owner đổi tài khoản/link Figma, phiên Codex hiện tại đã đọc được metadata UI Design và cả hai FigJam board mới bằng Figma MCP.
- PDF export vẫn là snapshot tham chiếu; khi làm UI/architecture phải ưu tiên link Figma/FigJam mới nếu MCP đọc được.
...
Tạo `frontend/apps/public-web`, `frontend/apps/clinic-admin`, `frontend/apps/owner-admin`, routing, guard, API client, tenant context.
...
5. Tạo shared UI package trong `frontend/packages/ui`.
6. Tạo shell cho Owner Super Admin trong `frontend/apps/owner-admin`.
7. Tạo shell cho Clinic Admin trong `frontend/apps/clinic-admin`.
8. Tạo shell cho Public Website trong `frontend/apps/public-web`.
