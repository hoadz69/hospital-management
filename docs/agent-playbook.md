...
- `Lead Agent: bắt đầu <task>`: intake + lane classification + team assembly; implement nếu plan/lane đã được duyệt rõ, nếu chưa thì tạo/update plan và dừng ở approval gate.
...
Lead report tối thiểu phải ghi: lane đã phân loại, agents đã tự chọn, Architect review gì, Frontend/Backend/DevOps/Database làm gì nếu có, QA verify gì, Documentation cập nhật docs nào, file dirty/untracked còn lại, commit split đề xuất nếu có, và xác nhận không stage/commit/push khi owner chưa yêu cầu.
